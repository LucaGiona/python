...
root = tk.Tk()
root.geometry("500x550+420+150")
root.title("message masker")

#stringVAr
selected_action = tk.StringVar()

def encrypt(text, s):  #s = Schlüssel
    result = ""
...
def decrypt(text, s):  #s = Schlüssel
    result = ""
    for i in range(len(text)):
        char = text[i]
        if char == " " or char == "." or char == ":" or char == "!" or char == "?":
            ...
            result += chr((ord(char) - s - 65) % 26 + 65)  #hier wird der Schlüssel nach links verschoben
        else:
            result += chr((ord(char) - s - 97) % 26 + 97)
    return result
...
def perform_action():
    if selected_action.get() == "verschlüsseln":
        ...
        output_entry.delete(0, tk.END)   #mögliche Voreingabe löschen Index 0 bis tk:END
...
input_text_scroll.grid(sticky="ew", column=1, row=1, pady=1)
...
action_rb1 = ttk.Radiobutton(input_frame, text="Verschlüsseln", variable=selected_action,
                             value="verschlüsseln")
action_rb1.grid(column=1, row=3, sticky="w")

action_rb2 = ttk.Radiobutton(input_frame, text="Entschlüsseln", variable=selected_action,
                             value="entschlüsseln")
